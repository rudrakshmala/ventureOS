...
import {
  Users, Mail, MessageSquare, Percent, Clock, Activity, Cpu, FolderGit2, LogOut,
  TrendingUp, DollarSign, Send, Trophy, ChevronLeft, ChevronRight, RefreshCw, Zap,
  Target, Play, Search, StopCircle, CheckCircle2, AlertCircle, X, MailOpen,
  Briefcase, Code2, LineChart
} from 'lucide-react';

interface Stats {
  sourced?: number; emailed?: number; follow_up_1?: number; follow_up_2?: number;
  replied?: number; closed_won?: number; closed_lost?: number; [key: string]: number | undefined;
}

interface EmpireStats { totalLeads: number; pitchesSent: number; dealsWon: number; totalRevenue: number; }

interface SalesLead {
  id: string; email: string; name: string | null; company: string | null;
  source: string; status: string; painPoint: string | null; pitchSent: string | null;
  repliedAt: string | null; createdAt: string;
}

interface SalesProject {
  id: string; clientEmail: string; clientName: string | null; brief: string;
  status: string; priceQuoted: number | null; paidAmount: number | null; createdAt: string;
}

interface Deal {
  id: string; title: string; value: number; currency: string; status: string;
  closedAt: string | null; paidAt: string | null; createdAt: string;
  lead: { contactEmail: string | null; authorUsername: string } | null;
}

interface AgentRun {
  id: string; runType: string; status: string; leadsFound: number; pitchesSent: number;
  dealsWon: number; revenue: number; startedAt: string; completedAt: string | null;
}
...
function fmtInr(paise: number | null | undefined) { return paise == null ? '-' : 'Rs.' + (paise / 100).toLocaleString('en-IN'); }
function fmtUsd(usd: number) { return '$' + usd.toLocaleString('en-US', { minimumFractionDigits: 0 }); }
function fmtDate(iso: string | null) { return !iso ? '-' : new Date(iso).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: '2-digit' }); }
function fmtTime(iso: string) { return new Date(iso).toLocaleTimeString(); }

function statusColor(status: string) {
  const s = status.toLowerCase();
  if (['won', 'paid', 'delivered', 'success', 'closed_won', 'replied'].includes(s)) return 'bg-emerald-950/50 text-emerald-400 border-emerald-800/40';
  if (['running', 'negotiating', 'building', 'emailed', 'sourced', 'follow_up_1', 'follow_up_2', 'scoping'].includes(s)) return 'bg-indigo-950/50 text-indigo-400 border-indigo-800/40';
  if (['failed', 'lost', 'closed_lost', 'bounced'].includes(s)) return 'bg-red-950/50 text-red-400 border-red-800/40';
  if (['intake', 'draft', 'pending'].includes(s)) return 'bg-zinc-800 text-zinc-300 border-zinc-700';
  return 'bg-amber-950/40 text-amber-400 border-amber-800/40';
}

function Badge({ label }: { label: string }) {
  return <span className={'px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border ' + statusColor(label)}>{label.replace(/_/g, ' ')}</span>;
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const tick = () => setTime(new Intl.DateTimeFormat('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }).format(new Date()) + ' IST');
    tick();
    const id = setInterval(tick, 1000);
    return () => clearInterval(id);
  }, []);

  const fetchOutreachStats = async () => { try { const r = await apiFetch('/api/v1/outreach/stats'); if (r.ok) setOutreachStats(await r.json()); } catch {} };
  const fetchEmpireStats = async () => { try { const r = await apiFetch('/api/v1/empire/stats'); if (r.ok) setEmpireStats(await r.json()); } catch {} };
  const fetchLeads = async () => { try { const r = await apiFetch('/api/v1/leads'); if (r.ok) setLeads(await r.json()); } catch {} };
  const fetchProjects = async () => { try { const r = await apiFetch('/api/v1/projects'); if (r.ok) setProjects(await r.json()); } catch {} };
  ...
  useEffect(() => {
    ...
    es.onmessage = (event) => {
      try {
        ...
        if (parsed.type === 'stats_update' && parsed.data && Object.keys(parsed.data).length > 0) setOutreachStats(parsed.data);
      } catch {}
    };
    return () => es.close();
  }, []);
  ...
  const handleLogout = () => window.location.href = window.location.protocol + '//logout:logout@' + window.location.host + window.location.pathname;
  ...
  const runScout = async () => {
    ...
    try {
      ...
      es.addEventListener('EMPIRE_LOG', (e: any) => addControlLog(JSON.parse(e.data).log));
      es.addEventListener('COMPLETE', () => { es.close(); setScoutState('done'); addControlLog('Scout complete.'); fetchLeads(); fetchOutreachStats(); fetchEmpireStats(); });
      es.addEventListener('CRASH', (e: any) => { es.close(); setScoutState('error'); addControlLog('Scout CRASHED: ' + JSON.parse(e.data).message); });
      es.onerror = () => { es.close(); setScoutState('error'); addControlLog('Scout connection lost.'); };
    } catch (err: any) { setScoutState('error'); addControlLog('Scout error: ' + err.message); }
  };
  ...
  const runOutreach = async () => {
    ...
    try {
      ...
      } else { setOutreachState('error'); addControlLog('Outreach start failed: ' + res.status); }
    } catch (err: any) { setOutreachState('error'); addControlLog('Outreach error: ' + err.message); }
  };
  ...
  const runEmpire = async () => {
    ...
    try {
      ...
      es.addEventListener('EMPIRE_LOG', (e: any) => addControlLog(JSON.parse(e.data).log));
      es.addEventListener('COMPLETE', () => { es.close(); setEmpireState('done'); addControlLog('Empire cycle complete!'); fetchLeads(); fetchOutreachStats(); fetchEmpireStats(); fetchAgentRuns(); });
      es.addEventListener('CRASH', (e: any) => { es.close(); setEmpireState('error'); addControlLog('Empire CRASHED: ' + JSON.parse(e.data).message); });
      es.onerror = () => { es.close(); setEmpireState('error'); addControlLog('Empire connection lost.'); };
    } catch (err: any) { setEmpireState('error'); addControlLog('Empire error: ' + err.message); }
  };
  ...
  return (
    <div className="min-h-screen p-6 md:p-10 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-zinc-800 pb-px">
        ...
        <button onClick={() => setActiveTab('executive')} className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 transition ${activeTab === 'executive' ? 'border-emerald-500 text-emerald-400 bg-emerald-500/10' : 'border-transparent text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800/50'}`}>
          <LineChart className="w-4 h-4" /> Executive Strategy
        </button>
      </div>

      {/* ================================================================================================= */}
      {/* 1. SALES & OUTREACH TAB */}
      {/* ================================================================================================= */}
      {activeTab === 'sales' && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <section className="card p-6 space-y-5 border border-sky-900/40 bg-sky-950/10">
            <div className="flex items-center gap-2">
              <Zap className="w-4 h-4 text-sky-400" />
              <h2 className="text-base font-semibold text-zinc-200">Sales Control Panel</h2>
              <span className="ml-2 text-[10px] uppercase tracking-wider bg-sky-950/60 text-sky-400 border border-sky-800/40 px-2 py-0.5 rounded">14 agents ready</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <button onClick={runScout} disabled={scoutState === 'running'} className="flex items-center justify-between gap-3 px-5 py-4 rounded-xl bg-zinc-900 border border-zinc-700 hover:border-indigo-600 hover:bg-indigo-950/30 disabled:opacity-50 transition group text-left">
                <div>
              ...
              <button onClick={runOutreach} disabled={outreachState === 'running'} className="flex items-center justify-between gap-3 px-5 py-4 rounded-xl bg-zinc-900 border border-zinc-700 hover:border-sky-600 hover:bg-sky-950/20 disabled:opacity-50 transition group text-left">
                <div>
                  <div className="flex items-center gap-2 text-sm font-semibold text-zinc-200 group-hover:text-sky-300 transition">
                    <Send className="w-4 h-4 text-sky-400" /> Run Outreach Cycle
                  </div>
                  <p className="text-[11px] text-zinc-500 mt-1">Source leads + send personalized cold emails</p>
                </div>
                {stateIcon(outreachState) || <Play className="w-4 h-4 text-sky-400 opacity-60 group-hover:opacity-100" />}
              </button>
            </div>
            {controlLog.length > 0 && (
              <div ref={controlLogRef} className="mt-2 h-36 overflow-y-auto bg-zinc-950 border border-zinc-800 rounded-lg p-3 font-mono text-[11px] text-zinc-400 space-y-0.5">
                {controlLog.map((line, i) => <div key={i} className={line.includes('CRASH') || line.includes('error') ? 'text-red-400' : line.includes('complete') || line.includes('done') ? 'text-emerald-400' : ''}>{line}</div>)}
              </div>
            )}
          </section>
          ...
          <section className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Emails Sent', value: emailsSent, Icon: Mail, color: 'text-sky-400' },
          ...
          <section className="card p-6 space-y-4">
            <h2 className="text-base font-semibold text-zinc-300 flex items-center gap-2">
              <Activity className="w-4 h-4 text-sky-400" /> Outreach pipeline funnel
            </h2>
            <div className="space-y-3">
              {[
                { label: 'Sourced', count: outreachStats.sourced || 0, color: 'bg-indigo-600' },
                { label: 'Emailed', count: outreachStats.emailed || 0, color: 'bg-sky-500' },
                { label: 'Follow Up 1', count: outreachStats.follow_up_1 || 0, color: 'bg-sky-400' },
                { label: 'Follow Up 2', count: outreachStats.follow_up_2 || 0, color: 'bg-sky-300' },
          ...
          <section className="card p-6 space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-semibold text-zinc-300 flex items-center gap-2">
                <Users className="w-4 h-4 text-sky-400" /> Sales leads <span className="ml-1 text-xs text-zinc-500 font-normal">({leads.length} total)</span>
              </h2>
            ...
            {leads.length === 0 ? (
              <div className="py-10 flex flex-col items-center justify-center text-center"><span className="text-zinc-600 block text-2xl mb-2">👥</span><span className="text-zinc-500">No leads sourced yet</span></div>
            ) : (
              <>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-zinc-800 text-[11px] font-semibold uppercase tracking-wider text-[var(--text2)]">
                        <th className="pb-3 pr-4 font-medium">Email</th><th className="pb-3 pr-4 font-medium">Company</th><th className="pb-3 pr-4 font-medium">Source</th><th className="pb-3 pr-4 font-medium">Status</th><th className="pb-3 pr-4 font-medium">Pain point</th><th className="pb-3 pr-4 font-medium">Mailbox</th><th className="pb-3 font-medium">Added</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-zinc-900/40 text-xs font-mono">
                      {pagedLeads.map((lead) => (
                        <tr key={lead.id} className="hover:bg-zinc-900/10 transition">
                          <td className="py-3 pr-4 text-zinc-300 font-medium">{lead.email}</td><td className="py-3 pr-4 text-zinc-400">{lead.company || '-'}</td><td className="py-3 pr-4"><span className="px-2 py-0.5 rounded text-[10px] bg-zinc-800 text-zinc-300 border border-zinc-700 uppercase">{lead.source}</span></td><td className="py-3 pr-4"><Badge label={lead.status} /></td><td className="py-3 pr-4 text-zinc-500 max-w-[180px] truncate" title={lead.painPoint || ''}>{lead.painPoint ? lead.painPoint.substring(0, 50) + (lead.painPoint.length > 50 ? '...' : '') : '-'}</td>
                          <td className="py-3 pr-4 text-zinc-400">
                            {lead.pitchSent ? (
                              <button onClick={() => setViewEmailLead(lead)} className="flex items-center gap-1.5 px-2 py-1 bg-zinc-800 hover:bg-zinc-700 hover:text-sky-300 rounded border border-zinc-700 transition"><MailOpen className="w-3.5 h-3.5" /> <span className="text-[10px] uppercase font-semibold">View</span></button>
                            ) : '-'}
                          </td>
          ...
          <section className="card p-6 space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-semibold text-zinc-300 flex items-center gap-2"><TrendingUp className="w-4 h-4 text-emerald-400" /> Deal pipeline <span className="ml-1 text-xs text-zinc-500 font-normal">({deals.length} total)</span></h2>
            ...
            {deals.length === 0 ? (
              <div className="py-10 flex flex-col items-center justify-center text-center"><span className="text-zinc-600 block text-2xl mb-2">🤝</span><span className="text-zinc-500">No deals yet</span></div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead><tr className="border-b border-zinc-800 text-[11px] font-semibold uppercase tracking-wider text-[var(--text2)]"><th className="pb-3 pr-4 font-medium">Deal</th><th className="pb-3 pr-4 font-medium">Contact</th><th className="pb-3 pr-4 font-medium">Value</th><th className="pb-3 pr-4 font-medium">Status</th><th className="pb-3 pr-4 font-medium">Closed</th><th className="pb-3 font-medium">Created</th></tr></thead>
                  <tbody className="divide-y divide-zinc-900/40 text-xs font-mono">
                    {deals.map((d) => (
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <section className="card p-6 space-y-5 border border-indigo-900/40 bg-indigo-950/10">
             <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-semibold text-zinc-200">Engineering Control Panel</h2>
              <span className="ml-2 text-[10px] uppercase tracking-wider bg-indigo-950/60 text-indigo-400 border border-indigo-800/40 px-2 py-0.5 rounded">59 agents ready</span>
            </div>
            <p className="text-sm text-zinc-400">Software engineering and delivery agents automatically wake up when a Deal is marked as PAID or when triggered from the Workspace Vault. Track active client projects below.</p>
          </section>

          <section className="card p-6 space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-semibold text-zinc-300 flex items-center gap-2"><FolderGit2 className="w-4 h-4 text-indigo-400" /> Client projects <span className="ml-1 text-xs text-zinc-500 font-normal">({projects.length} total)</span></h2>
            ...
            {projects.length === 0 ? (
              <div className="py-10 flex flex-col items-center justify-center text-center"><span className="text-zinc-600 block text-2xl mb-2">📁</span><span className="text-zinc-500">No projects yet</span></div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead><tr className="border-b border-zinc-800 text-[11px] font-semibold uppercase tracking-wider text-[var(--text2)]"><th className="pb-3 pr-4 font-medium">Client</th><th className="pb-3 pr-4 font-medium">Brief</th><th className="pb-3 pr-4 font-medium">Status</th><th className="pb-3 pr-4 font-medium">Quoted</th><th className="pb-3 pr-4 font-medium">Paid</th><th className="pb-3 font-medium">Date</th></tr></thead>
                  <tbody className="divide-y divide-zinc-900/40 text-xs font-mono">
                    {projects.map((p) => (
                      <tr key={p.id} className="hover:bg-zinc-900/10 transition">
                        <td className="py-3 pr-4 text-zinc-300">{p.clientEmail}</td><td className="py-3 pr-4 text-zinc-400 max-w-[200px] truncate" title={p.brief}>{p.brief.substring(0, 55)}{p.brief.length > 55 ? '...' : ''}</td><td className="py-3 pr-4"><Badge label={p.status} /></td><td className="py-3 pr-4 text-zinc-300">{fmtInr(p.priceQuoted)}</td><td className="py-3 pr-4 text-emerald-400 font-semibold">{fmtInr(p.paidAmount)}</td><td className="py-3 text-zinc-500">{fmtDate(p.createdAt)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {/* ================================================================================================= */}
      {/* 3. EXECUTIVE STRATEGY TAB */}
      {/* ================================================================================================= */}
      {activeTab === 'executive' && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <section className="card p-6 space-y-5 border border-emerald-900/40 bg-emerald-950/10">
            <div className="flex items-center gap-2">
              <LineChart className="w-4 h-4 text-emerald-400" />
              <h2 className="text-base font-semibold text-zinc-200">Executive Control Panel</h2>
              <span className="ml-2 text-[10px] uppercase tracking-wider bg-emerald-950/60 text-emerald-400 border border-emerald-800/40 px-2 py-0.5 rounded">3 C-suite agents ready</span>
            </div>
            
            <button onClick={runEmpire} disabled={empireState === 'running'} className="w-full flex items-center justify-between gap-3 px-5 py-4 rounded-xl bg-zinc-900 border border-zinc-700 hover:border-emerald-600 hover:bg-emerald-950/20 disabled:opacity-50 disabled:cursor-not-allowed transition group text-left">
              <div>
                <div className="flex items-center gap-2 text-sm font-semibold text-zinc-200 group-hover:text-emerald-300 transition">
                  <Cpu className="w-4 h-4 text-emerald-400" /> Full Empire Cycle (Trigger 76 Agents)
                </div>
                <p className="text-[11px] text-zinc-500 mt-1">Execute the complete top-to-bottom corporate workflow: Strategy, Sales, Development, QA, Deployment.</p>
              </div>
          ...
          <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div key={label} className="metric-card">
                <div className="flex justify-between items-start"><span className="text-xs font-semibold uppercase tracking-wider text-[var(--text2)]">{label}</span><Icon className={`w-4 h-4 ${color}`} /></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Activity log */}
            <section className="card p-6 flex flex-col h-[400px]" onMouseEnter={() => setIsHovered(true)} onMouseLeave={handleMouseLeave}>
              <div className="flex justify-between items-center mb-3">
                <h2 className="text-base font-semibold text-zinc-300 flex items-center gap-2"><Zap className="w-4 h-4 text-emerald-400" /> Live Activity Log</h2>
                <div className="flex items-center gap-2">
              ...
              <div className="flex-1 overflow-y-auto space-y-2 font-mono text-xs pr-1">
                {logs.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center p-4"><span className="text-zinc-600 block text-base mb-1">📡</span><span className="text-zinc-500 block">Waiting for events…</span></div>
                ) : (
                  logs.map((log) => (
                    <div key={log.id} className="p-2 rounded bg-zinc-950 border border-zinc-900/60 flex flex-col gap-0.5">
                      <div className="flex justify-between text-[10px] text-zinc-500"><span className="font-semibold text-zinc-400">{log.type}</span><span>{fmtTime(log.timestamp)}</span></div>
                      {log.data && Object.keys(log.data).length > 0 && <pre className="text-zinc-400 text-[10px] whitespace-pre-wrap break-all leading-relaxed">{JSON.stringify(log.data, null, 2)}</pre>}
                    </div>
                  ))
                )}
              </div>
            ...
            <section className="card p-6 flex flex-col h-[400px]">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-base font-semibold text-zinc-300 flex items-center gap-2"><Cpu className="w-4 h-4 text-emerald-400" /> Agent run history</h2>
              ...
              {agentRuns.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center"><span className="text-zinc-600 block text-2xl mb-2">🤖</span><span className="text-zinc-500">No agent runs logged yet</span></div>
              ) : (
                <div className="flex-1 overflow-x-auto overflow-y-auto">
                  <table className="w-full text-left border-collapse">
                    <thead><tr className="border-b border-zinc-800 text-[11px] font-semibold uppercase tracking-wider text-[var(--text2)]"><th className="pb-3 pr-4 font-medium">Run type</th><th className="pb-3 pr-4 font-medium">Status</th><th className="pb-3 pr-4 font-medium">Deals</th><th className="pb-3 font-medium">Started</th></tr></thead>
                    <tbody className="divide-y divide-zinc-900/40 text-xs font-mono">
                      {agentRuns.map((run) => (
                        <tr key={run.id} className="hover:bg-zinc-900/10 transition">
                          <td className="py-3 pr-4 text-zinc-300 font-semibold">{run.runType.replace(/_/g, ' ')}</td><td className="py-3 pr-4"><Badge label={run.status} /></td><td className="py-3 pr-4 text-emerald-400">{run.dealsWon}</td><td className="py-3 text-zinc-500">{fmtDate(run.startedAt)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <section className="card p-6 space-y-5">
            <h2 className="text-base font-semibold text-zinc-300 flex items-center gap-2"><Activity className="w-4 h-4 text-emerald-400" /> Global agent memory bus</h2>
            {memory.length === 0 ? (
              <div className="py-10 flex flex-col items-center justify-center text-center"><span className="text-zinc-600 block text-2xl mb-2">🧠</span><span className="text-zinc-500">No shared memory entries yet</span></div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead><tr className="border-b border-zinc-800 text-[11px] font-semibold uppercase tracking-wider text-[var(--text2)]"><th className="pb-3 pr-4 font-medium">Agent ID</th><th className="pb-3 pr-4 font-medium">Key</th><th className="pb-3 pr-4 font-medium">Value preview</th><th className="pb-3 font-medium">Updated</th></tr></thead>
                  <tbody className="divide-y divide-zinc-900/40 text-xs font-mono">
                    {memory.map((entry, idx) => {
                      ...
                      return (
                        <tr key={idx} className="hover:bg-zinc-900/10 transition">
                          <td className="py-3 pr-4 font-semibold text-emerald-400">{entry.agentId}</td><td className="py-3 pr-4 text-zinc-300">{entry.key}</td><td className="py-3 pr-4 text-zinc-400 max-w-sm truncate" title={valStr}>{valStr.substring(0, 60)}{valStr.length > 60 ? '...' : ''}</td><td className="py-3 text-zinc-500">{fmtTime(entry.updatedAt)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-zinc-950 border border-zinc-800 rounded-xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800 bg-zinc-900/50">
              <div>
                <h3 className="font-semibold text-zinc-200 flex items-center gap-2"><MailOpen className="w-4 h-4 text-sky-400" /> Sent Mail</h3>
                <p className="text-xs text-zinc-500 mt-1">To: <span className="text-zinc-300 font-mono">{viewEmailLead.email}</span></p>
              </div>
              <button onClick={() => setViewEmailLead(null)} className="p-1.5 rounded-lg hover:bg-zinc-800 text-zinc-400 transition"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-5 overflow-y-auto flex-1">
              {viewEmailLead.pitchSent ? (
                <div className="prose prose-invert prose-sm max-w-none prose-p:leading-relaxed prose-a:text-sky-400 p-4 bg-zinc-900 rounded-lg border border-zinc-800/60" dangerouslySetInnerHTML={{ __html: viewEmailLead.pitchSent }} />
              ) : (
